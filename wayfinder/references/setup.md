...
- **RPC overrides**: Optional chain-specific RPC URLs under `strategy.rpc_urls` (if omitted, on-chain calls use Wayfinder’s RPC proxy at `system.api_base_url` with your API key)
